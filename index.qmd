...
```{r study_area_map, message=F, warning=F, cache=T}
#| fig-align: center
#| fig-margin: 0
...
```{r ipums_bg_data_setup, message=F, warning=F}
#| cache: false
...
```{r ipums_bg_data_processing, message=F, warning=F}
#| cache: false
...
```{r pw_areal_interpolation, message=F, warning=F}
#| cache: false
...
```{r glmmtmb_regression, message=F, warning=F}
#| cache: false
...
* u~i~        -> random intercept for block group i (GEOID)
...
```{r diagnostic_testing, message = FALSE, warning = FALSE}
#| cache: false
...
```{r model_accuracy, message=F, warning=F}
#| cache: false
...
```{r random_effects, message=T, warning=T}
#| cache: false
...
18. Jokela, M., Batty, G. D., Vahtera, J., Elovainio, M., & Kivimäki, M. (2013).
...
24. Partridge, M. D., Rickman, D. S., Ali, K., & Olfert, M. R. (2008).
