...
st_regions <- list(
  'West Region' = c('Chautauqua', 'Cattaraugus', 'Allegany'),
  'Central Region' = c('Steuben', 'Schuyler', 'Chemung'),
  '8 Region' = c('Tompkins', 'Cortland', 'Tioga', 'Chenango',
                 'Broome', 'Otsego', 'Schoharie', 'Delaware')
)
...
st_civil <- 
  st_read('data/boundaries/NYS_Civil_Boundaries.geojson') %>%
  filter(COUNTY %in% southern_tier_counties) %>%
  st_transform(crs = 26918)

st_cities <-
  st_read('data/boundaries/NYS_City_Boundaries.geojson') %>%
  filter(COUNTY %in% southern_tier_counties) %>%
  st_transform(crs = 26918)

st_towns <-
  st_read('data/boundaries/NYS_Town_Boundaries.geojson') %>%
  filter(COUNTY %in% southern_tier_counties) %>%
  st_transform(crs = 26918)
...
```{r distress_vars_plot, message = FALSE, warning = FALSE, cache = TRUE, results = FALSE}
...
```{r county_distressed_analysis, message=F, warning=F, cache=T, results = FA:SE}
...
```{r land_cover_change_analysis, message=F, warning=F, cache=T, results = FALSE}
...
ranef_map <-
  ...
  geom_sf_text(data = nys_cities, aes(label = NAME), 
               color = 'black', angle = 22.5, size = 2.8) +
  geom_sf_text(data = st_civil %>% filter(NAME %in% c("Alfred", "Bath", "Cooperstown", "Watkins Glen", "Hammondsport")), aes(label = NAME), color = 'black', angle = -22.5, size = 2) +
