---
title: "Assessing Distress Risk of New York's Southern Tier Using Socioeconomic Indicators and Land Cover Changes (2009-2023)"
...
The Southern Tier of New York, comprising fourteen counties designated as part of Northern Appalachia by the Appalachian Regional Commissiono (ARC), has long faced significant socioeconomic challenges. These counties have experiences some of the steepest population declines in New York State outside of New York City, driven by prolonged economic restructuring and out-migration (McMahon, 2024; Johnson & Lichter, 2019). Historically reliant on manufacturing, agriculture, and extractive industries, the region has struggled to adapt to post-industrial economic realities. As a result, many communities across the Southern Tier endure persistently high poverty rates and low median incomes relative to national benchmarks (ARC, 2023).
...
Primary land cover data for years between 2008 and 2023 come from the [Multi-Resolution Land Characteristics Consortium](https://www.mrlc.gov/data/project/annual-nlcd)'s National Land Cover Database and were downloaded programmatically from their [Data page](https://www.mrlc.gov/data?f[0]=category%3ALand%20Cover). This data was processed using the [terra](https://rspatial.github.io/terra/) package.
...
The map below shows the 14 counties in New York's Southern Tier (Allegany, Broome, Cattaraugus, Chautauqua, Chemung, Chenango, Cortland, Delaware, Otsego, Schoharie, Schuyler, Steuben, Tioga, and Tompkins) and all 308 census tracts (in 2020-2023).

MAKE A GGPLOT EQUIVALENT TO BELOW MAP
...
study_counties <- counties(state = 'NY', cb = TRUE, year = 2020) %>%
  filter(NAME %in% southern_tier_counties)

study_tracts <- tracts(state = 'NY', county = southern_tier_counties, cb = TRUE, year = 2020)
...
# create scatter plot for each year of avg family income and pct below poverty
distressed_vars_plt <-
  st_bgs %>%
  st_drop_geometry() %>%
  group_by(GEOID) %>%
  summarize(tot_families = sum(tot_families),
            agg_fam_income = sum(agg_fam_income),
            tot_pop_pov_count = sum(tot_pop_pov_count),
            pop_in_poverty = sum(pop_in_poverty),
            avg_fam_income = agg_fam_income / tot_families,
            pct_below_poverty = pop_in_poverty / tot_pop_pov_count) %>%
  ggplot() +
    geom_point(aes(x = avg_fam_income, y = pct_below_poverty, color = factor(county))) +
    scale_x_log10()

distressed_vars_plt

# MAKE A BIVARIATE CHLOROPLETH MAP OF THE 2 VARIABLES

```

Add some text about the plot and map
...
This section pertains to downloading and processing national land cover rasters over the entire 14-county study area.  ADD MORE HERE  The interpretation of the land cover images' pixel values and corresponding land cover classes is from the [Multi-Resolution Land Characteristics Consortium](https://www.mrlc.gov/data/type/land-cover).
...
nlcd_wrapped <- lapply(lc_yrs, function(yr) {
  ...
  lc_rast <- paste(mrlc_data_dir, paste('nlcd_', as.character(year), sep = ''), sep = '')
  
  # catch errors in processing the raster
  tryCatch({
    # load raster, then crop and mask to southern tier study area
    r <- rast(lc_rast)
    r_crop <- terra::crop(r, vect(st_transform(st_full_study_area, crs = st_crs(r))))
    r_mask <- terra::mask(r_crop, vect(st_transform(st_full_study_area, crs = st_crs(r))))
    
    # add year as name of masked raster
    names(r_mask) <- paste('NLCD_', as.character(year), sep = '')
    
    # return wrapped, masked raster
    return(
      wrap(
        project(r_mask, crs(as_spatvector(st_bgs_final)))
      )
    )
  }, error = function(e) {
...
nlcd_stack.reclass <- classify(nlcd_stack, rcl = lc_type_reclass, others = NA)
...
## County-Level Change (2009-2023)

Add some text here

```{r county_distressed_analysis, message=F, warning=F, cache=T}
...
# create gt table of changes between 2009-2023
st_counties %>%
...
A map of changes between the 2009 and 2023 reclassified NLCD rasters to see what areas changed, and how they changed

Add some text here

```{r land_cover_change_analysis, message=F, warning=F, cache=T}
...
Dependent variable = is_distressed (0 if not distressed, 1 if distressed)
Independent variables = tot_pop_density + pct_didnt_work_past_yr + lag_pct_didnt_work_past_yr + avg_rent + pct_divorced + pct_developed + factor(year_group) + (1 | GEOID)

There are 15,060 observations in the dataset, which represent the 1,004 uniform block groups across a 15-year period. The model attempts to predict the distressed status of a block group given the scaled predictors.
...
The KS test result for uniformity showed no significant deviation from uniformity (p = 0.108), which suggests expected well-behaced residuals. The dispersion test showed no significant over- or under-dispersion (Disperson = 0.907, p = 0.088), signalling appropriate levels of variance. The outlier test showed no significant outliers (p = 0.252). The zero inflation test showed no evidence of zero inflation (p = 0.472). The QQ plot indicates appropriate residual distribution, and the Residuals vs. Predicted plot shows a slight curve at higher predictions, but is overall flat otherwise. All VIF values used to measure multicollinearity are less than 5, meaning there is no concerning multicollinearity between the predictors.

While significant spatial autocorrelation is present in the residuals (Moran's I = 0.117, p < 0.001), its magnitude is modest and expected given the spatial clustering of socioeconomic distress. In a precursor models, Moran's I was approximately 0.15. The inclusion of a lagged variable and block group-level random effects substantially reduced spatial dependence compared to these initial models.

Overall, the model fits well, the random effects have a reasonable distribution, and multicollinearity is not a concern. While spatial autocorrelation is present, it is moderate and expected in socioeconomic spatial data. There were also attempts at limiting spatial autocorrelation in the form of introducing a spatially lagged covariate, which alleviated some of the autocorrelation. Remaining autocorrelation likely reflects unmeasured spatial processes beyond the scope of this analysis.
...
fixed_effects <-
  data.frame(
    ...
    CI_Lower = lower_ci,
    CI_Upper = upper_ci,
    p_value = summary(model_lag)$coefficients$cond[, 'Pr(>|z|)']
  )
...
Overall, the model has a prediction accuracy rate of about 95%. However, this is mostly due to the model's high level of ability at ruling out distress evidenced by it's 98% accuracy at predicting no distress. Accuracy suffers a bit at predicting block groups that are in distress, where it is less than 70% accurate. This suggests that the model is conservative and is cautious about labeling a block group as distressed unless there's strong evidence.
...
A variance of 8.71 and standard deviation of 2.951 in the random effects shows a high level of between-group variability, which suggests that unobserved block group-level factors play a large role in predicting distress in this model. It was thus justified to include the random intercepts as a well to account for this heterogeneity.

The map highlights spatial variation in unexplained distress risk across block groups. Several areas exhibit significantly higher baseline distress risk (in red) even after controlling for socioeconomic, demographic, and built environment predictors. This suggests the presence of unmeasured local factors contributing to persistent distress. Conversely, pockets of lower-than-expected risk (in blue) indicate potential resilience factors that buffer against socioeconomic vulnerabilities. These spatial patterns warrant further investigation to identify contextual drivers of distress beyond those captured in the current model.



# Discussion and Conclusions

The inclusion of a spatially lagged unemployment variable highlights significant spatial interdependence in socioeconomic distress. Block groups are not only affected by their internal economic conditions but also by those of their surrounding areas, consistent with theories of spatial spillover and neighborhood effects.

The spatial distribution of random intercepts across New York’s Southern Tier reveals substantial unexplained variation in distress risk. Elevated baseline distress in specific block groups—particularly in former industrial centers—suggests that structural and historical factors, not fully captured by current socioeconomic indicators, continue to shape vulnerability. Conversely, some block groups exhibit resilience despite adverse socioeconomic profiles, indicating the presence of unmeasured protective factors. These patterns highlight the importance of considering localized, place-based dynamics in addressing socioeconomic distress within the region.

Other interpolation methods: Interpolating data using population-weighted centroid assignment can contribute to higher absolute differences between original count data and interpolated data, as shown by Hallisey et al. (2017). Combined population and areal weighting could potentially be employed for more accurate variable estimates in the interpolation process.


Improve Sensitivity: An ROC curve analysis to explore different thresholds?


Other distressed classification systems (<https://www.congress.gov/crs-product/IF12074>)
...
3. Bivand, R. S., & Piras, G. (2015). Comparing implementations of estimation methods for spatial econometrics. Journal of Statistical Software, *63*(18), 1–36. <https://doi.org/10.18637/jss.v063.i18>

4. Brooks M.E., Kristensen K., van Benthem K.J., et al. (2017). glmmTMB Balances Speed and Flexibility Among Packages for Zero-inflated Generalized Linear Mixed Modeling. *The R Journal*, *9*(2), 378–400. <https://doi.org/doi:10.32614/RJ-2017-066>.

5. Dormann, C. F., McPherson, J. M., Araújo, M. B., et al. (2007). Methods to account for spatial autocorrelation in the analysis of species distributional data: A review. *Ecography*, *30*(5), 609–628. <https://doi.org/10.1111/j.2007.0906-7590.05171.x>

6. Ellis, G.F., Burk, D., Roberts, F. (2025). ipumsr: An R Interface for Downloading, Reading, and Handling IPUMS Data. R package version 0.8.2, <https://github.com/ipums/ipumsr>, <https://www.ipums.org>, <https://tech.popdata.org/ipumsr/>. 

7. Flowerdew, R., & Green, M. (1992). Developments in areal interpolation methods and GIS. The Annals of Regional Science, 26(1), 67–78. <https://doi.org/10.1007/BF01581870>

8. Galili, T., O'Callaghan, A., Sidi, J., & Sievert, C. (2017). heatmaply: an R package for creating interactive cluster heatmaps for online publishing. *Bioinformatics*, *34*(9), 1600-1602. <https://doi.org/10.1093/bioinformatics/btx657>

9. Gelman, A. & Hill, J. (2007). Data Analysis Using Regression and Multilevel/Hierarchical Models. Cambridge University Press. https://doi.org/10.1017/CBO9780511790942

9. Goodchild, M. F., & Lam, N. S. (1980). Areal interpolation: A variant of the traditional spatial problem. *Geo-Processing*, *1*(3), 297-312.

10. Gregory, I. N. (2002). The accuracy of areal interpolation techniques: standardising the crosswalk. *Computers, Environment and Urban Systems*, *26*(3), 293-314.

11. Hallisey, E., Tai, E., Berens, A., et al. (2017). Transforming geographic scale: a comparison of combined population and areal weighting to other interpoaltion methods. *International Journal of Health Geographics*, *16*(29). <https://doi.org/10.1186/s12942-017-0102-z>

12. Hartig, F. (2024). DHARMa: Residual Diagnostics for Hierarchical (Multi-Level / Mixed) Regression Models. R package version 0.4.7, <https://github.com/florianhartig/dharma>.

13. Hijmans, R. (2025). terra: Spatial Data Analysis. R package version 1.8-6. <https://github.com/rspatial/terra>

14. Johnson, K. M., & Lichter, D. T. (2019). Rural depopulation: Growth and decline processes over the past century. Rural Sociology, 84(1), 3–27. <https://doi.org/10.1111/ruso.12266>

15. Jokela, M., Batty, G. D., Vahtera, J., Elovainio, M., & Kivimäki, M. (2013).
...
14. K C, S., Gyawali, B. R., Lucas, S., Antonious, G. F., Chiluwal, A., & Zourarakis, D. (2024). Assessing Land-Cover Change Trends, Patterns, and Transitions in Coalfield Counties of Eastern Kentucky, USA. *Land*, *13*(9), 1541. <https://doi.org/10.3390/land13091541>

15. Ludke, R. L., Obermiller, P. J., & Rademacher, E. W. (2012). Demographic Change in Appalachia: A Tentative Analysis. *Journal of Appalachian Studies*, *18*(1/2), 48–92. <http://www.jstor.org/stable/23337708>

16. Manson, S., Schroeder, J., Van Riper, D., et al. (2024). IPUMS National Historical Geographic Information System: Version 19.0 [dataset]. Minneapolis, MN: IPUMS. <http://doi.org/10.18128/D050.V19.0>.

17. McMahon, E.J. (2024). Eight in 10 New York towns and cities have lost population since 2020. *Empire Center*. <https://www.empirecenter.org/publications/eight-in-10-new-york-towns-and-cities-have-lost-population-since-2020>

18. Partridge, M. D., Rickman, D. S., Ali, K., & Olfert, M. R. (2008).
...
18. Thiede, B. C., Lichter, D. T., & Slack, T. (2018). Working, but poor: The good life in rural America? Journal of Rural Studies, 59, 183–193. <https://doi.org/10.1016/j.jrurstud.2016.02.007>

18. U.S. Geological Survey. (2024). Annual National Land Cover Database (NLCD) Collection 1 Science Products (2008–2023) [Data set]. Multi-Resolution Land Characteristics Consortium (MRLC). <https://www.mrlc.gov/sites/default/files/docs/LSDS-2103%20Annual%20National%20Land%20Cover%20Database%20%28NLCD%29%20Collection%201%20Science%20Product%20User%20Guide%20-v1.0%202024_10_15.pdf>

19. Walker, K. (2023). "7.3 Small area time-series analysis," in *Analyzing US Census Data: Methods, Maps, and Models in R*. CRC Press: Boca Raton, Florida, <https://walker-data.com/census-r/spatial-analysis-with-us-census-data.html#small-area-time-series-analysis>

20. Walker, K. & Herman, M. (2024). tidycensus: Load US Census Boundary and Attribute Data as 'tidyverse' and 'sf'-Ready Data Frames. R package version 1.6.6. <https://walker-data.com/tidycensus>

21. Zuur, A. F., Ieno, E. N., Walker, N. J., Saveliev, A. A., & Smith, G. M. (2009). Mixed Effects Models and Extensions in Ecology with R. Springer Science & Business Media. <https://doi.org/10.1007/978-0-387-87458-6>
...
* 2009 & 2010 Per Capita Income and Haven't Worked Population: https://apps.bea.gov/iTable/?reqid=70&step=30&isuri=1&major_area=0&area=xx&year=2018,2017,2016,2015,2014,2013,2012,2011,2010,2009,2008,2007,2006,2005,2004,2003,2002,2001,2000,1999,1998,1997,1996&tableid=21&category=421&area_type=0&year_end=-1&classification=non-industry&state=0&statistic=3&yearbegin=-1&unit_of_measure=levels
